...
_default_path = r'/opt/dbeaver/dbeaver'
...
class AppletApplication(BaseApplication):
    ...
    def get_app_work_path():
        ...
        return r'/%s/.local/share/DBeaverData' % current_username
    ...
    def init_driver(self):
        src_driver = os.path.join(current_path, 'drivers')
    ...
    def init_other_config(self):
        ...
        core_config_file = os.path.join(config_path, 'org.jkiss.dbeaver.core.prefs')

        config = self._read_config(core_config_file)
        ...
        self._write_config(core_config_file, config)

        statistics_config_file = os.path.join(config_path, 'org.jkiss.dbeaver.ui.statistics.prefs')
        config = self._read_config(statistics_config_file)
        config['feature.tracking.skipConfirmation'] = 'true'
        self._write_config(statistics_config_file, config)
    ...
    def run(self):
        ...
        if function is None:
            ...
            params = function()
        subprocess.run([self.path, '-con', params])
